...
class LiveShadowIngestTests(unittest.TestCase):
    ...
    def test_v3_lord_state_is_readable_but_v6_starts_a_fresh_sequence(self):
        ...
        self.assertEqual(ledger.fdr_state("shadow-confirmation-v6:equity")["tests"], 1)
    ...
    def test_crash_retry_reuses_v6_fdr_decision_when_resolution_changes(self):
        cid = self.candidate["candidate_id"]
        self._rows(cid, [2.0] * 8)
        self._rows(self.baseline["candidate_id"], [0.0] * 8)
        self._rows(f"shadow:null:{cid}", [-1.0] * 8)
        ...
        def record_then_crash(ledger, scope, test_id, p_value, *, alpha=.05):
            result = original_record(ledger, scope, test_id, p_value,
                                     alpha=alpha)
            if str(scope) == "shadow-confirmation-v6:equity" and not crashed["value"]:
                crashed["value"] = True
        ...
        # at 30k; the immutable tail key must still reuse the one v6 row.
        self.assertEqual(result["ingested"], 1, result)
        state = FactoryLedger(self.edge_path).fdr_state(
            "shadow-confirmation-v6:equity")
        self.assertEqual(state["tests"], 1)
    ...
    def test_live_authorization_fails_closed_when_durable_fdr_is_tampered(self):
        ...
        with sqlite3.connect(self.edge_path) as db:
            ...
            db.execute(
                ...
                "FROM runs WHERE run_id=?)", (0.5, "shadow-confirmation-v6:equity", run_id))
        self.assertFalse(self.ledger.eligibility(cid)["eligible"])
