...
It is a measurement tool, not an authorizing one.  It replays frozen specs, it
never tunes, proposes, promotes, or writes to a ledger, and both arms use the
...
def _arm(rows: Sequence[Mapping[str, Any]], *, starting_cash: float) -> ArmResult:
    ...
    return ArmResult(
        ...
        signal_opportunities=sum(
            bool(row.get("signal_opportunity")) for row in rows) + len(executed))
...
def run_cost_rerun(
    ...
    _raw, bars, snapshot_map, quote_rows = _read_discovery_rows(
        corpus, require_provenance=False,
    ...
    schedule = measure_quote_costs(
        quotes if isinstance(quotes, list) else list(quotes),
    ...
    for spec in cohort:
        ...
        for name, model in (("configured", configured), ("measured", measured)):
            ...
            row[name] = _arm(account["rows"], starting_cash=starting_cash).as_dict()
        results.append(row)
