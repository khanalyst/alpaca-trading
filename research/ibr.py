...
from .market_data import NEW_YORK, OptionSnapshot, UnderlyingBar
...
def _execution_price(reference: float, direction: str, *, entry: bool,
    ...
    spread = cfg.spread_bps / 20_000.0
...
def _trade_from_exit(*, vehicle: str, symbol: str, day: date, direction: str,
                     ...
                     multiplier: int = 1, entry_timestamp: datetime | None = None) -> IBRTrade:
    entry_price = _execution_price(entry_reference, direction, entry=True, cfg=cfg)
    exit_price = _execution_price(exit_reference, direction, entry=False, cfg=cfg)
    ...
    return IBRTrade(
        ...
        stop_price=entry_reference * (1 - cfg.stop_pct if direction == "long" else 1 + cfg.stop_pct),
        target_price=entry_reference * (1 + cfg.target_pct if direction == "long" else 1 - cfg.target_pct),
...
def _replay_session(bars: Sequence[UnderlyingBar], *, vehicle: str, symbol: str,
    ...
    range_bars = [b for b in bars if start <= _local(b.timestamp, zone) < range_end
                  and b.end <= range_end and b.identity.as_of <= range_end]
    ...
    post = [b for b in bars if b.end > range_end
    ...
    signal_idx = next((i for i, b in enumerate(post)
                       if b.high > high or b.low < low), None)
    ...
    if signal.identity.as_of > signal.end or entry_bar.identity.as_of > entry_bar.timestamp:
        return None
    long_break = signal.high > high
    short_break = signal.low < low
    ...
    if vehicle == "option":
        if option_snapshots is None:
            return None
        snap = option_snapshots.get(entry_bar.timestamp)
        if snap is None:
            eligible = [s for ts, s in option_snapshots.items() if ts <= entry_bar.timestamp]
            snap = max(eligible, key=lambda s: s.timestamp) if eligible else None
        if snap is None or snap.ask <= 0:
            return None
        if snap.identity.as_of > entry_bar.timestamp:
            return None
        entry_ref = snap.ask if direction == "long" else (snap.bid or snap.last or 0)
        if entry_ref <= 0:
            return None
        multiplier = snap.contract.multiplier
    stop = entry_ref * (1 - cfg.stop_pct if direction == "long" else 1 + cfg.stop_pct)
    target = entry_ref * (1 + cfg.target_pct if direction == "long" else 1 - cfg.target_pct)
    for bar in post[signal_idx + 1:]:
        ...
        if gap_stop or gap_target:
            ...
            return _trade_from_exit(vehicle=vehicle, symbol=symbol, day=day, direction=direction,
                                    ...
                                    exit_bar=bar, exit_reference=bar.open,
        ...
        if hit_stop or hit_target:
            ...
            return _trade_from_exit(vehicle=vehicle, symbol=symbol, day=day, direction=direction,
                                    range_high=high, range_low=low, signal=signal,
                                    entry_bar=entry_bar, entry_reference=entry_ref,
                                    exit_bar=bar, exit_reference=level,
    ...
    boundary = next((b for b in post if _local(b.timestamp, zone) >= close_at), None)
    if boundary is not None:
        last = boundary
        exit_ref = last.open
    else:
        candidates = [b for b in post if b.end <= close_at]
        if not candidates:
            return None
        last = candidates[-1]
        exit_ref = last.close
    if vehicle == "option" and option_snapshots:
        eligible = [s for ts, s in option_snapshots.items() if ts <= last.timestamp]
        if eligible:
            snap = max(eligible, key=lambda s: s.timestamp)
            exit_ref = snap.bid if direction == "long" else (snap.ask or snap.last or exit_ref)
    return _trade_from_exit(vehicle=vehicle, symbol=symbol, day=day, direction=direction,
                            range_high=high, range_low=low, signal=signal,
                            entry_bar=entry_bar, entry_reference=entry_ref,
...
class IBRReplay:
    """State-free object wrapper for callers that prefer a replay instance."""

    def __init__(self, config: IBRConfig | None = None) -> None:
        self.config = config or IBRConfig()

    def run(self, bars: Iterable[UnderlyingBar], **kwargs) -> IBRResult:
        return replay_ibr(bars, config=self.config, **kwargs)


backtest_ibr = replay_ibr


__all__ = [
    "IBRConfig", "IBRReplay", "IBRResult", "IBRTrade", "ReplayError",
    "backtest_ibr", "replay_ibr", "replay_ibr_vehicles",
]
