...
| Effective equity stop geometry | Accepted | The stop floor is `max(30 bps, active stressed-cost scenario / max-cost-to-risk ratio)`. When binding, the authored stop/target plan is vetoed before sizing; neither leg is rewritten, and authored/effective geometry, scenario, ratio, and binding are retained as telemetry. |
